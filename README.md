...
&\quad\times \underbrace{p(\alpha_{i} \mid \tau_{\alpha})p(\tau_{\alpha})}_{\propto p(\tau_{\alpha} \mid \alpha_{i})} \cdot \underbrace{p(\beta_{k} \mid \tau_{\beta})p(\tau_{\beta})}_{\propto p(\tau_{\beta} \mid \beta_{k})} \cdot \underbrace{p(\gamma_{t} \mid \tau_{\gamma})p(\tau_{\gamma})}_{\propto p(\tau_{\gamma} \mid \gamma_{t})}\\
&\quad\times \underbrace{p(p_{i} \mid \tau_{p})p(\tau_{p})}_{\propto p(\tau_{p} \mid p_{i})} \cdot \underbrace{p(q_{k} \mid \tau_{q})p(\tau_{q})}_{\propto p(\tau_{q} \mid q_{k})}\\
&\quad\times \underbrace{p(\omega) \cdot p(\delta)}_{\text{global intercept}}\\
&\quad\times p(\sigma)
\end{aligned}$$
